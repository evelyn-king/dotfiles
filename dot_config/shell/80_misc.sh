...
if [[ "${SHELL_CONFIG_STAGE:-interactive}" == "interactive" ]]; then
  ...
  if [ -f "${HOME}/.local/opt/google-cloud-sdk/path.${shell_config_hook_shell}.inc" ]; then
    . "${HOME}/.local/opt/google-cloud-sdk/path.${shell_config_hook_shell}.inc"
  fi

  # The next line enables shell command completion for gcloud.
  if [ -f "${HOME}/.local/opt/google-cloud-sdk/completion.${shell_config_hook_shell}.inc" ]; then
    . "${HOME}/.local/opt/google-cloud-sdk/completion.${shell_config_hook_shell}.inc"
  fi

  # obsidian cli - macOS only for now
