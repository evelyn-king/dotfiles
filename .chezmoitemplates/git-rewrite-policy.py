...
BLOCKED_PATTERNS = [
    ...
    (
        ...
        "gh pr merge is not allowed - the user must merge PRs themselves, never the agent",
    ),
]

PROTECTED_BRANCHES = {"main", "master"}
PROTECTED_QUOTED_TOKENS = {
    "--all",
    "--amend",
    "--force",
    "--force-with-lease",
...
def preserve_policy_token(match: re.Match[str]) -> str:
...
def strip_quoted_strings(command: str) -> str:
    """Remove quoted prose and heredocs while retaining policy tokens.

    Shell quoting does not make a flag or ref safe. Preserve those tokens so
    `git push origin "main"` is checked like its unquoted form.
    """
    ...
    # Remove double-quoted strings, preserving policy tokens.
    result = re.sub(r'"(?:[^"\\]|\\.)*"', preserve_policy_token, result)

    # Single-quoted strings cannot contain escaped single quotes.
    result = re.sub(r"'[^']*'", preserve_policy_token, result)
