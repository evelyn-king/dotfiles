...
```bash
sudo darwin-rebuild switch --flake "$(chezmoi source-path)/nix#macbook"
...
hardcoded list goes stale on the next cask change. Start a clean Bash session for
these commands so the cask list splits into separate arguments even when your
login shell is zsh. Nix formats the list without requiring Python or jq:

```bash
/bin/bash --noprofile --norc
casks=$(nix eval --raw \
  "$(chezmoi source-path)/nix#darwinConfigurations.macbook.config.homebrew.casks" \
  --apply 'cs: builtins.concatStringsSep " " (map (c: c.name) cs)')
printf '%s\n' "$casks"
...
The last command lists declared casks Homebrew does not yet manage, including
missing applications and possible collisions. Continue in the same Bash session
only after Nix evaluation succeeds and you have reviewed the list:
...
Run this after installing Nix and Homebrew and before the first activation.
Homebrew attempts to adopt existing app bundles, installs missing casks, and
skips casks it already manages. Adoption can reject a different bundle version
for an app that does not self-update, and `pkg` artifacts still run their
installers. Resolve each failure before activation. This procedure has not yet
passed an existing-Mac migration test. To skip a cask, remove its declaration
from `nix/flake.nix` before activation.
