...
To restore the repo configuration after a refresh, run this in a terminal.
Chezmoi asks how to handle files changed since its last apply. Review the diff
and choose `overwrite` for each managed file whose repo version you want back:

```bash
chezmoi apply
chezmoi status --exclude=scripts .config/hypr
chezmoi diff --exclude=scripts .config/hypr
hyprctl reload
hyprctl configerrors
```

An unattended apply cannot answer those prompts. After reviewing the changes,
you can explicitly overwrite just the three managed files without running
package or other scripts:

```bash
chezmoi diff --exclude=scripts \
  ~/.config/hypr/hyprland.lua ~/.config/hypr/bindings.lua ~/.config/hypr/looknfeel.lua
# Run only after deciding to restore these files from the repo.
