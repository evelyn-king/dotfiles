...
- Omarchy-only state is gated off every other OS by `.chezmoiignore`, which keys on `ID=omarchy`. It comes in three parts: plain files (`dot_config/omarchy/`, `dot_config/hypr/`, `dot_config/mimeapps.list`, `dot_config/xdg-terminals.list`, `dot_local/share/applications/`), declarative selections in `.chezmoidata/omarchy.yaml`, and the `run_after_omarchy-*.sh.tmpl` scripts that apply them. Only files that diverge from `/usr/share/omarchy/config/` are tracked; see `docs/omarchy.md`.
